...
class LiveWebinarController extends Controller
{
	...
	public function index(Request $request) {
		try {
			...
			->where('webinars.webinar_type','=', config('constants.WEBINAR_TYPE.LIVE'))
			->active()->orderby('start_time', 'ASC')->paginate(config('constants.API.DEFAULT_PAGE'));

			if ($liveWebinars->count() > 0) {
	            foreach($liveWebinars as $key => $row) {
	                $webinarList[$key] = [
	                    'id' => $row->id,
