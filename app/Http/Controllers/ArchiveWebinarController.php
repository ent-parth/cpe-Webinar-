...
class ArchiveWebinarController extends Controller{
	...
	public function index(Request $request){
		...
		$webinars = Webinar::select('webinars.id','speakers.first_name','speakers.last_name','webinars.title', 'webinars.fee','webinars.webinar_type','webinars.recorded_date','webinars.start_time', 'webinars.end_time', 'webinars.created_at', 'webinars.status')->leftJoin('speakers','speakers.id','=','webinars.created_by');
		...
		if($status_type){
			...
			$status = $request->input('webinar_type');	
		}


        //print_r($status); die;
        if ($status != '' && $status == 'active') {
        	$webinars = $webinars->where('webinars.webinar_type', '=', 'archived');
        ...
        elseif ($status != '' && $status == 'inactive') {
            $webinars = $webinars->where('webinars.webinar_type', '=', 'archived');
        	$webinars = $webinars->where('webinars.status', '=', 'inactive');
        }elseif ($status != '' && $status == 'draft') {
            $webinars = $webinars->where('webinars.webinar_type', '=', 'live');
           // $webinars = $webinars->where('webinars.status', '=', 'active');
        }
		...
		$webinar_archive_atype = array('live','archived');

		$webinars = $webinars->WhereIn('webinar_type',$webinar_archive_atype)->orderby('start_time', 'assc')->paginate(env('PAGINATION'));
		//$webinars = $webinars->where('webinar_type','live')->orderby('start_time', 'asc')->paginate(50);
		...
		if ($request->ajax()) {
			return Response::json(View('backEnd/archive_webinar/index-ajax', compact('webinars','statusList','status'))->render());
		}
		
        return view('backEnd.archive_webinar.index', compact('webinars','statusList'));
    }
	...
	public function destroy($id,Request $request){
        try {
			if(is_numeric($id) && !empty($id)){
				...
				if($delete){
					$request->session()->flash('success', __('Archive webinar has been deleted successfully.'));
				} else {
					$request->session()->flash('error', __('Archive webinar can not be deleted. Please try again.'));
				}
				return redirect()->route('archive-webinar',$request->input('uri'));
			}else{
				$request->session()->flash('error', __('Archive webinar can not be deleted. Please try again.'));
				 return redirect()->route('archive-webinar',$request->input('uri'));
			}	
        ...
        try {
            if (!empty($request->id)) {
                ...
                if ($webinar) {
                    ...
                    $data = ['success' => 'Archive webinar has been deleted successfully.'];
                    
                   
                } else {
                    $data = ['error' => 'Archive webinar can not be deleted. Please try again.'];
                }
            } else {
                $data = ['error' => 'Archive webinar can not be deleted. Please try again.'];
            }
	...
	* View Archive Webinars status 
     ...
     */
	 ...
	 public function view(Request $request, $id){
		  $LiveWebinarsView = Webinar::select('webinars.id','webinars.title','webinars.cpa_credit', 'webinars.image' , 'webinars.reason','webinars.learning_objectives','webinars.Instructional_method', 'webinars.fee','webinars.webinar_type','webinars.presentation_length','webinars.webinar_transcription','webinars.description','webinars.subject_area','webinars.course_level','webinars.pre_requirement','webinars.advance_preparation','who_should_attend','webinars.faq_1','webinars.faq_2','webinars.faq_3','webinars.faq_4','webinars.faq_5','webinars.recorded_date','webinars.start_time', 'webinars.end_time','webinars.tag', 'webinars.created_at','webinars.created_by', 'webinars.status','speakers.first_name','speakers.last_name','webinars.video','webinars.documents','webinars.time_zone')
									 ...
									 ->first();
		  //dd($LiveWebinarsView);
		  return view('backEnd.archive_webinar.view', compact('LiveWebinarsView'));
	 }
    ...
    public function edit($id) {
        try{
            if(is_numeric($id) && !empty($id)){
                ...
                if ($webinar) {
                    return view('backEnd.archive_webinar.edit', compact('webinar', 'statusList','subject_area_id','course_level_id','tag_id', 'courseLevel', 'subjectAria', 'tags','userType','who_should_attend_array'));
                }else{
                    $request->session()->flash('error', __('Archive Webinar not available. Please, try again.'));
                    return redirect()->route('/archive-webinar').$request->input('uri');
                }
            }else{
                $request->session()->flash('error', __('Archive Webinar could not be load. Please, try again.'));
                return redirect()->route('/archive-webinar').$request->input('uri');
            }
        } catch (Exception $ex) {
            abort(404);
    ...
    public function update(Request $request) { 
        ...
        try {
            ...
            if($updateData){
                $request->session()->flash('success', __('Archive Webinar has been update successfully.'));
                //return redirect()->route('selfstudy-webinar',$request->input('uri'));
                return redirect('/archive-webinar?'.$uri);
            }
            $request->session()->flash('error', __('Archive Webinar could not be saved. Please, try again.'));
        } catch (Exception $exc) {
            $request->session()->flash('error', __('Archive Webinar could not be saved. Please, try again.'));
        }
    }
    ...
    public function statusUpdate($id,$status,Request $request) {
		$updated_at = Carbon::now();
		if($status=='inactive'){
			$update_active = Webinar::where('id', '=', $id)->update(['status' => 'active','updated_at'=>$updated_at]);
			$request->session()->flash('success', __('Archive Webinar status updated successfully.'));
		}elseif($status == 'active'){
			$update_inactive = Webinar::where('id', '=', $id)->update(['status' => 'inactive','updated_at'=>$updated_at]);
			$request->session()->flash('success', __('Archive Webinar status updated successfully.'));
		}
        return redirect()->back();
    }
    ...
    public function store_video(Request $request) {
        try {
           ...
           if($video){
            ...
            $addVideoArray = ['video'=>$videoURL,'status'=>'inactive','webinar_type'=>'archived'];
            $updateData = Webinar::where('id',$webinar_id)->update($addVideoArray);                
            if($updateData){
            	$request->session()->flash('success', __('Archive Webinar video has been added successfully.'));
            	if(!empty($actionType)){
					return Response::json(['success' => true,'latestVideo'=>$videoName], 200);
				}else{
					return redirect('/archive-webinar');
				}
            ...
            $request->session()->flash('error', __('Archive Webinar video could not be saved. Please, try again.'));
        } catch (Exception $exc) {
            $request->session()->flash('error', __('Archive Webinar video could not be saved. Please, try again.'));
        }
    }
    ...
    public function videoStatusUpdate(Request $request) { 
        ...
        if(!empty($id)){ 
            if($status == 'active' && empty($code)){
                ...
                $url = 'https://api.vimeo.com/oauth/authorize?client_id='.env("VIMEO_CLIENTS_KEY").'&response_type=code&redirect_uri='.env("HTTP_ADMIN_APP_URL").'archive-webinar/update-video-status?id='.$id.'&state=create&scope=upload';
                
                $notification_text = 'Your webinar has been accepted from the admin.';
                $link = 'archive-webinar/view/'.encrypt($id); 
                ...
                $url = 'https://api.vimeo.com/oauth/authorize?client_id='.env("VIMEO_CLIENTS_KEY").'&response_type=code&redirect_uri='.env("HTTP_ADMIN_APP_URL").'archive-webinar/update-video-status?id='.$id.'&state='.$status.'&scope=delete';
                ...
                if($state == 'create'){
                    ...
                    curl_setopt_array($curl, array(
                        ...
                        CURLOPT_POSTFIELDS => "------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name=\"grant_type\"\r\n\r\nauthorization_code\r\n------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name=\"code\"\r\n\r\n".$code."\r\n------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name=\"redirect_uri\"\r\n\r\n".env("HTTP_ADMIN_APP_URL")."archive-webinar/update-video-status?id=".$id."\r\n------WebKitFormBoundary7MA4YWxkTrZu0gW--",
                    ...
                    if ($err) {
                        ...
                        if(!empty($decodeJsonResponce) && !empty($decodeJsonResponce->access_token)){
                            ...
                            if(!empty($webinarData)){
								...
								//$videoLink = 'http://user.spaarg.tech/uploads/webinar_video/1544704332videoplayback.mp4';
                                ...
                                if($err){
                                    ...
                                    if(!empty($decodeVimeoArray) && !empty($decodeVimeoArray->link)){
                                        ...
                                        return redirect()->route('archive-webinar');  
                                    }else{
