...
class CourseController extends Controller{
    ...
    public function index(Request $request){ 
		...
		//home page filter for Topic of interest
		if(is_array($request->input('tag')) && !empty($request->input('tag'))){
            $webinars = $webinars->whereIn('webinars.tag',$request->input('tag'));
		}
		
		//home page filter for Field of Study
		if(is_array($request->input('field_of_study')) && !empty($request->input('field_of_study'))){
            $webinars = $webinars->whereIn('webinars.subject_area',$request->input('field_of_study'));
		}
		...
		//dd($webinarArray);
        return view('user.course.index',compact('webinars','subjectAria','companies','webinarArray' ,'tagLists'));
    }
    ...
    public function detail(Request $request,$string=NULL){
    	...
    	if(is_numeric($id) && !empty($id)){
			$webinar = Webinar::Select('webinars.id','webinars.view_count','webinars.title','webinars.webinar_type','webinars.description','webinars.fee','webinars.time_zone','webinars.recorded_date',
			...
			if(!empty($webinar)){
				...
				$liveWebinars = Webinar::Select('id','title','start_time','webinar_type')
										->where('status','active')
				...
				if ($request->ajax()) {
					return view('user.course.detail',compact('id','webinar','resultStatus','liveWebinars','webinarQuestions','selfStudyWebinars','reviewResultArray','webinarQusReviewResults','reviewResultStatus','webinarQusFinalResults','finalResultArray','finalResultStatus','displayFinalExam','selfStudyVideoDuration','type','finalExamResult','finalPer','webinarRegisters'));		
				}
				return view('user.course.detail',compact('id','webinar','resultStatus','liveWebinars','webinarQuestions','selfStudyWebinars','reviewResultArray','webinarQusReviewResults','reviewResultStatus','webinarQusFinalResults','finalResultArray','finalResultStatus','displayFinalExam','selfStudyVideoDuration','type','finalExamResult','finalPer','webinarRegisters'));		
			}else{
    ...
    public function storeQuestion(Request $request){
		...
		if(!empty($webinarId) && is_numeric($webinarId) && !empty($type)){
			...
			if(count($webinarQuestionss)>0){
				...
				return Response::json(View('user/course/review-question-ajax', compact('resultStatus','id','webinarQuestions','webinarQusReviewResults','reviewResultArray','reviewResultStatus','webinarQusFinalResults','finalResultArray','finalResultStatus','displayFinalExam','selfStudyVideoDuration','type','finalExamResult','finalPer','webinar'))->render());
			}else{
					...
					if(empty($webinarRegister)){
						...
						if(empty($checkWebinarRegister)){
							...
							if(empty($webinarDetail->fee) || $webinarDetail->fee == NULL){
								...
								return redirect('course-register-payment/'.encrypt($webinar_id));
							}
						}else{
    ...
    public function webinarRegisterPayment($webinar_id){
		if(!empty($webinar_id)){
			...
			return view('user.course.make-payment',compact('webinar','webinarRegister'));		
		}else{
			Session::put('mycpa_message_error', 'Something went wrong, please try again.');
				...
				if($stripeAmount == $webinar->fee){
					...
					if($error){
						...
						if(isset($result['status']) == 'succeeded' && isset($result['amount']) && !empty($result['amount'])){
							...
							return Redirect('course-register-payment/'.encrypt($webinarId)); 
						}
					}	
				}else{
	...
	* Create new attendees for webinar.
    ...
    /**
     * store like user wise 
     *
     * @param $request
     * @return response
     */
	public function webinarLike(Request $request) {
		$user_id = Session::get('mycpa_client_id');
		$webinar_id   = $request->input('webinar_id');
		$checkLike = WebinarLike::select('id')->where('webinar_id','=',$webinar_id)->where('user_id','=',$user_id)->count();
		if($checkLike == '0') {
			$created_at = Carbon::now();
			$webinarInsert = ['webinar_id'=>$webinar_id,
							'user_id'=>$user_id,
							'created_at'=>$created_at
						  ];
			$likeInser = WebinarLike::create($webinarInsert);
			return Response::json(['msg' => 'like'], 200);
		} else {
			WebinarLike::where('webinar_id','=',$webinar_id)->where('user_id','=',$user_id)->delete();
			return Response::json(['msg' => 'unlike'], 200);
		}
    }
}
