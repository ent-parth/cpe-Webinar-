...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('body').on('click', '#add-import, #add-state, .edit-state', function (e) {
        if (popupOpen == false) {
            popupOpen = true;
            $('#add-state-modal').load($(this).attr("data-url"), function (result) {
                $('#add-state-modal').modal({show: true});
            });
        }
    });
    ...
    $("#add-state-modal").on("hidden.bs.modal", function () {
        popupOpen = false;
    });
});
