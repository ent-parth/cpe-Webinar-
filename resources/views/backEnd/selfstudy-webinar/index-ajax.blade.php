...
<tr id="filter_tr{{$webinar->id}}">
  ...
  <td><select name="series" class="series" onchange = "updateSeries(this.value,'<?php echo $webinar->id?>')";>
    ...
    <a href="{{route('selfstudy-webinar.view',$webinar->id)}}{{(Request::getQueryString() ? ('?' .  Request::getQueryString()) : '')}}" class="btn btn-group btn-default btn-sm" title="Manage Webinar">Manage Webinar</a> <a href="{{route('selfstudy-webinar.edit', $webinar->id)}}{{(Request::getQueryString() ? ('?' .  Request::getQueryString()) : '')}}" class="btn btn-group btn-default btn-sm" title="Edit"><i class="fa fa-edit"></i></a> 
    <!--<a href="{{route('selfstudy-webinar.delete',$webinar->id)}}{{(Request::getQueryString() ? ('?' .  Request::getQueryString()) : '')}}" onclick="return confirm('Are you sure you want to delete.?');" title="Delete" class="btn btn-group btn-default btn-sm"><i class="fa fa-trash-o"></i></a>--></td>
</tr>
@empty
<tr>
  <td colspan="7"><center>
