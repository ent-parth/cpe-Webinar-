...
<div class="login-box-body">
  ...
  {{ Form::open(['name' => 'reset', 'id'=>'reset-password-form', 'class' => 'form-validate','route' => ['company.reset.password']]) }}
  ...
  <div class="form-group has-feedback">
    ...
    <span class="glyphicon glyphicon-lock form-control-feedback"></span> @if ($errors->has('password_confirmation'))
    <label class="validation-invalid-label">{{ $errors->first('password_confirmation') }}</label>
    @endif </div>
  <div class="row"> 
    <!--.col-->
    <div class="col-xs-6 pull-right"> {{ Form::button('Change Password',array('class'=>'btn btn-primary btn-block btn-flat','type'=>'submit','title'=>'Change Password')) }} </div>
    <!--.col--> 
  </div>
  {{ Form::close() }} </div>
@endsection
