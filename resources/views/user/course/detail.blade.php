...
@section( 'content' )
@php  
CommonHelper::courseCount($webinar->id); 
@endphp
<style type="text/css">
.color-red{color:#F00;}
  ...
  <div class="container-fluid">
    <div class="d-md-flex aside-area justify-content-md-between">
      <main class="">
          ...
          <div class="post-block"> 
            @if(Session::get('mycpa_client_id'))
                ...
                @else
                	...
                	<a href="{{env('APP_URL').'/course-register/'.encrypt($webinar->id)}}{{(Request::getQueryString() ? ('?' .  Request::getQueryString()) : '')}}"><input class="btn-primary btn" value="Register" type="button"></a>
                @endif    
            @else
            	<p>You have to register to see video.</p>
                <a href="{{route('client.register')}}"><input class="btn-primary btn" value="Register" type="button"></a>
            @endif    
          </div>
        </div>
        ...
        @if($webinar->webinar_type == 'self_study')  <!--&& !empty(Session::get('mycpa_client_id')-->
              <div class="tab-row"> 
              <!-- Nav pills -->
              <ul class="my-tabs">
                <li> <a @if(isset($type) && $type == 'review') class="active" @endif data-target="#rquestions" href="javascript:void(0);">Review Questions</a> </li>
                <li> <a @if(isset($type) && $type == 'final') class="active" @endif  data-target="#qfinal" href="javascript:void(0);">Final Exam</a> </li>
              </ul>
              
              <!-- Tab panes -->
              <div class="tab-contents">
                  <div id="responReviewQuestion">
                    @include('user/course/review-question-ajax')
                  </div>
            </div>
        @endif
      ...
      <aside>
        <div class="cource-detail-box">
     	...
     	@if(!empty(Session::get('mycpa_client_id')) && empty($webinarRegisters))
            ...
            <form action="{{env('APP_URL')}}/course-payment" id="stripeForm" method="POST" class="">
                ...
                <input type="button" id="stripe-button" class="btn btn-secondary" value="TAKE THIS COURSE"/>
            </form>
            @else
            	<a href="{{env('APP_URL').'/course-register/'.encrypt($webinar->id)}}{{(Request::getQueryString() ? ('?' .  Request::getQueryString()) : '')}}" class="btn btn-secondary">TAKE THIS COURSE</a>
            @endif
        @endif
        @if(empty(Session::get('mycpa_client_id')))
        ...
        <div class="aside-box">
          <h3>Live CPE Webinars</h3>
          	@if(count($liveWebinars) > 0)
            	@foreach($liveWebinars as $liveWebinar)
                    <div class="card">
                        <h4>{{$liveWebinar->title}}</h4>
                        <p>{{date("l, F d, Y",strtotime($liveWebinar->start_time))}}</p>
                        <a href="{{env('APP_URL')}}/course-detail/{{encrypt($liveWebinar->id)}}" class="btn-text">View Details/Register</a> 
                    </div>
                @endforeach
          	@else
            	<div class="card">
                	<p>No live CPE webinar</p>
                </div>	
            @endif
        </div>
        <div class="aside-box">
          ...
          @if(count($selfStudyWebinars) > 0)
            	@foreach($selfStudyWebinars as $selfStudyWebinar)
                    <div class="card">
                        ...
                        <a href="{{env('APP_URL')}}/course-detail/{{encrypt($selfStudyWebinar->id)}}" class="btn-text">View Details/Register</a> 
                    </div>
                @endforeach
          	@else
            	<div class="card">
